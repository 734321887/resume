...
- **离职原因：** 阶段性交付完成后的业务调整。
- **SkillRL（Harness/经验沉淀框架）：** 主导从 0→1 架构设计与落地，形成「SkillCreate 冷启动 → AutoAgent 经验蒸馏 → ClawEval 确定性评测 → Skill Hub 检索复用」闭环；以内部真实任务（机械臂抓取）作为持续反馈源，成功率约 60%→85%。
- **评测与回归：** 建立可复现的沙箱评测与 AB/回归机制（不依赖 LLM 自评），将“策略生成—评测—回流更新”变成日常迭代流水线，支撑模型/策略与 Harness 的共同进化。
- **多智能体与 AgentOS：** 牵头巡检行业「数据 OS + AgentOS 底座、快慢双系统、多智能体协同、端边云执行、驾驶舱管控」分层架构；以「虚拟沙盒迭代 + 真实场景校验」打通数字 Agent 与物理执行环节，覆盖电力、化工、安防、轨交等场景。

### 遥望科技（A 股上市）｜ 产研总监　　2025.07 – 2026.01

- **离职原因：** 主业下滑导致 AI 投入收缩。
- **AIP（组织级 Agent Harness）：** 负责产品与技术底座，确立「组织智慧中枢」定位，交付 SOP 编排、Plan 引擎、资源协同（云盘/知识库/数据库）与多角色协作机制。
- **Context & Plan：** 面向复杂任务实现上下文组织、动态调整、暂停与重跑；通过连接池与扫描策略优化，将复杂任务上下文调度耗时从数十秒级降至毫秒级。
- **Memory & Persona：** 设计短期缓存 + 长期记忆双层结构；角色专属知识库 + 元知识库优先级调度；多角色画像与记忆隔离，保障跨会话一致性；数据一致性保障率约 99%。

### 广东明日联合 ｜ 高级 AI 技术总监　　2025.04 – 2025.07

- **离职原因：** 公司突发治理变故。
- **对话交互链路：** 重构低算力场景引导式对话算法（Context / Prompt Engineering），多轮对话引擎覆盖 12 种疾病；单轮可澄清信息约 3 项→约 12 项；边缘设备约 200ms 响应，科室推荐准确率约 65%→90%。
- **工程与成本：** 结合剪枝与 INT8 量化方案，将能力稳定运行在老旧边缘设备；建立对话回归用例与效果评估流程，保证上线迭代质量。
- **快速交付体系：** 搭建低代码平台与 AI 工具链，支撑 12 个项目并行开发，周期缩短约 60%、研发成本降低约 45%。
